...
export const GET = async (request: Request) => {
    try {
        ...
        // reciveing coupple of things from client
        ...
        if(!categoryId || !Types.ObjectId.isValid(categoryId)){
            return new NextResponse(JSON.stringify({message: 'Invalid or missing catogryId'}), {status: 400})
        }
        ...
        // modifying the startDate and endDate
        ...
        // skip is a built in function provided bu mongoose. means that on a particular page how many blogs need to be skipped.
        const skip = (page -1) * limit;

        // we can fetch multiple blogs
        const blogs = await Blog.find(filter).sort({created: 'asc'})
        .skip(skip)
        .limit(limit);
...
export const POST = async(request: Request) => {
    try {
        ...
        // creating new Instance for blog
