...
export const PATCH = async(request: Request, context:{params: any}) => {
    ...
    try {
        ...
        return new NextResponse('Error in uodating category' + error.message, {
            status: 500
        })
    }
}
...
export const DELETE = async(request: Request, context: {params: any}) => {
    ...
    try {
        ...
        const category = await Category.findOne({_id:categoryId, user: userId});
        if(!category){
            return new NextResponse(JSON.stringify({message: 'Category not found or does not belong to the user'}), {status: 404})
        }
